...
public class Asignatura {
  ...
  // El orden en el que se especifican los atributos static y final no es
  // relevante
  ...
  public Asignatura(String nombre, int maxEstudiantes) {
    ...
    // Los atributos numéricos se inicializan a 0 por defecto, no es necesario
    // inicializar numeroEstudiantes a 0
    numeroEstudiantes = 0;
  }
  ...
  public boolean addEstudiante(String nombre, String apellidos, String nia) {
    ...
    // Una vez validados los argumentos, creamos el nuevo objeto de la clase
    // Estudiante
  ...
  public boolean addNota(int posicionEstudiante, int posicionEvaluacion, float nota) {
    ...
    // Si la posición en la que vamos a almacenar la nota ya tiene un valor,
    // mostramos
    // un mensaje de aviso
    ...
    // Una vez validados los argumentos, almacenamos la nota proporcionada en la
    // posición
  ...
  /*
   * El método notasEstudianteToString(int posicionEstudiante) nos va a facilitar
   * imprimir
  ...
  public String notasEstudianteToString(int posEstudiante) {
    ...
    for (int posEvaluacion = 0; posEvaluacion < NUMERO_EVALUACIONES; posEvaluacion++) {
      if (notas[posEstudiante][posEvaluacion] == null) {
        ...
        // Si no tenemos la nota de alguna evaluación, ya no calculamos la nota final
        // ponderada
    ...
    if (imprimirNotaFinal) {
      ...
      // Si no tenemos la nota final, imprimimos un asterisco, pero mantenemos la
      // tabulación
  ...
  /*
   * El método listaEstudiantesToString() imprime el listado numerado de
   * estudiantes registrados.
   * Su sitio más adecuado sería la clase Main porque es parte de la interfaz, no
   * añade
  ...
  /*
   * Método toString() que debe imprimir todos los detalles de la asignatura, con
   * las
